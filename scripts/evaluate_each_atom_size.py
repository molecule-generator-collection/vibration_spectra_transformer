...
def main(BATCH_SIZE, MODEL_NUMBER, atom_size, device):
    LR = 1e-5
    DATA_DIRECTORY = f"data_directory/GDB9/word_level_tokenized_{atom_size}"
    ...
    model_params = {
        ...
        "embed_dimention": 128, #same as decoder
    ...
    BASE_DIRECTORY = str(PROJECT_ROOT / "results")
    ...
    training_params = {
        ...
        "validation_step_interval_rate": 1, #validation interval during a training epoch
        ...
        "num_epochs": 10000,
        "patience": 1000000000000, #almost infinite value to ignore
        ...
        "module_directory_path": str(PROJECT_ROOT / "modules"),
    ...
    print(src_test_freq[0])
    print(src_test_IR[0])
    print(src_test_Raman[0])
    print(src_test_attention_masks[0])
    data_size = src_test_freq.size(0)
    ...
    #save
    with open(f"{BASE_DIRECTORY}/{MODEL_DIRECTORY}/{SAVE_DIRECTORY_NAME}/evaluation_result_test_atomSize{atom_size}.txt_{BATCH_SIZE}", "w") as f:
...
if __name__ == "__main__":
    for model_number in ["Drop0_LR4_2", "Drop0_LR4_3"]:
        for atom_size in [4, 5, 6, 7, 8, 9]:
