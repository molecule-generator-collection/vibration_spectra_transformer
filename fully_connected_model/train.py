#!/usr/bin/env python
"""Train the isolated fully connected spectrum-to-SMILES baseline."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default=str(DEFAULT_RESULTS_DIRECTORY))
    ...
    parser.add_argument("--latent-dimension", type=int, default=128)
...
def loss_for_batch(model, batch, device, criterion) -> torch.Tensor:
    freq, ir, raman, spectrum_mask, smiles_ids, _ = [item.to(device) for item in batch]
    logits = model(freq, ir, raman, spectrum_mask)
    return criterion(logits.transpose(1, 2), smiles_ids[:, 1:].long())
...
def train(args: argparse.Namespace, distributed: DistributedContext) -> None:
    ...
    raw_model = FullyConnectedSmilesPredictor(model_params).to(device)
    model = (
    ...
    if distributed.is_main:
        ...
        print(
            ...
            f"valid={len(valid_data):,}; batch_per_gpu={args.batch_size}; "
            f"global_batch={args.batch_size * distributed.world_size}; "
            f"parameters={parameter_count(raw_model):,}"
        )
